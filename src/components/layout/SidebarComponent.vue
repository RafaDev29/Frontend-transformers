<template>
  <v-navigation-drawer v-model="model" app permanent :width="rail ? 80 : 320" :rail="rail"
    ...
    <div
      class="bg-gradient-to-br from-slate-50 to-slate-200 dark:from-gray-800 dark:to-gray-900 border-b border-slate-300 dark:border-gray-600 relative overflow-hidden">
      <!-- Overlay decorativo -->
      <div
    ...
    <div class="p-3 flex-1 overflow-y-auto">
      ...
      <div v-if="maintenanceItems.length > 0" class="mt-6">
        ...
        <div
          class="bg-gradient-to-br from-green-50/80 to-emerald-50/60 dark:from-green-900/20 dark:to-emerald-900/15 border border-green-200/40 dark:border-green-700/30 rounded-2xl p-3 shadow-lg shadow-green-100/50 dark:shadow-green-900/20 backdrop-blur-sm">
            ...
            <v-tooltip :disabled="!rail" :text="item.title" location="end">
              <template #activator="{ props: tooltipProps }">
                <router-link v-bind="rail ? tooltipProps : {}" :to="item.to"
                  ...
                  <!-- Icono más pequeño -->
                  <div
                    class="w-7 h-7 rounded-lg flex items-center justify-center flex-shrink-0 transition-all duration-300 relative z-10"
                    :class="$route.path === item.to
                  ...
                  <!-- Texto más compacto -->
                  <div v-if="!rail" class="flex-1 min-w-0">
                    <span class="text-xs font-medium tracking-wide transition-colors duration-300" :class="$route.path === item.to
                      ? 'text-colorDark1 dark:text-color4'
                  ...
                  <!-- Efecto hover sutil -->
                  <div
                    class="absolute inset-0 bg-gradient-to-r from-transparent via-color2/8 to-transparent dark:via-color3/10 opacity-0 hover:opacity-100 transition-opacity duration-300 pointer-events-none rounded-lg" />
                </router-link>
              </template>
            </v-tooltip>
          </div>
