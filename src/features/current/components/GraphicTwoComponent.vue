          ...
          if (ch1Val != null) {
            html += `
              <div style="display: flex; align-items: center; gap: 8px;">
                ...
                <span style="font-weight: 600; color: #1e7f14;">${ch1Val.toFixed(2)}A</span>
              </div>
            `
          }
          ...
          if (ch2Val != null) {
            html += `
              <div style="display: flex; align-items: center; gap: 8px;">
                ...
                <span style="font-weight: 600; color: #dc2626;">${ch2Val.toFixed(2)}A</span>
              </div>
            `
          }
        ...
        hooks: {
          draw: [
            (u) => {
              ...
              ctx.fillText(`Mínimo: ${min.toFixed(1)}A`, u.bbox.left + u.bbox.width - 10, minY + 5)

              // Etiqueta máxima
              ctx.fillStyle = '#10b981'
              ctx.fillRect(u.bbox.left + u.bbox.width - 120, maxY - 14, 110, 26)
              ctx.fillStyle = '#ffffff'
              ctx.fillText(`Máximo: ${max.toFixed(1)}A`, u.bbox.left + u.bbox.width - 10, maxY + 5)
              ctx.restore()
            }
          ]
        }
