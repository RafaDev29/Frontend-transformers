        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
          ...
          <div>
            ...
            <input id="alertName" 
                   ...
                   placeholder="Ej: Tensión Crítica - Whatsapp" 
                   required />
...
const getDescriptionPlaceholder = () => {
  if (form.alertType === 'whatsapp') {
    return 'Ej: Esta es una alerta de WhatsApp que se envía cuando la tensión supera los límites establecidos...'
  } else if (form.alertType === 'email') {
    return 'Ej: Esta es una alerta de Email que notifica sobre anomalías detectadas en el sistema...'
  }
...
const validateForm = () => {
  ...
  // Validar contactos adicionales si tienen valor
...
const handleSubmit = () => {
  if (validateForm()) {
    ...
    const dataToSend = {
      ...
      alertDescription: form.alertDescription,
      sendToFactory: form.sendToFactory,
      sendToClient: form.sendToClient,
      additionalContacts: form.additionalContacts.filter(contact => contact.value.trim() !== ''),
...
// Limpiar contactos adicionales cuando cambia el tipo de alerta
